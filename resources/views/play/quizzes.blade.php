            ...
            <form method="POST" action="{{ route('categories.quizzes.answer', ['categoryId' => $categoryId]) }}">
                ...
                <div class="lg:w-2/3 w-full mx-auto overflow-auto">
                    <table class="table-auto w-full text-left whitespace-no-wrap">
                        <thead>
                            <tr>
                                <th class="whitespace-nowrap px-4 py-3 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100 rounded-tl rounded-bl">番号</th>
                            ...
                            @for ($i = 0; $i < count($quiz['options']); $i++)
                                <tr>
                                    ...
                                    <td class="w-10 text-center">
                                        <input name="optionId[]" type="checkbox" value="{{ $quiz['options'][$i]['id'] }}" class="form-checkbox h-5 w-5 text-indigo-600">
                                    </td>
                                </tr>
                            @endfor
                        </tbody>
